...
<div class="flex items-center justify-between mb-6">
    <p class="text-gray-500 text-sm">{{ $articles->total() }} artikel</p>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left">
                ...
                <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Dibuat</th>
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-5 py-3">
                    ...
                    <p class="text-gray-400 text-xs">{{ $article->slug }}</p>
                </td>
                <td class="px-5 py-3 text-center">
                    <span class="inline-block {{ $article->is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }} text-xs font-semibold px-2.5 py-1 rounded-full">
                        {{ $article->is_published ? 'Publish' : 'Draft' }}
                    </span>
                </td>
                <td class="px-5 py-3 text-gray-400 text-xs">{{ $article->created_at->format('d M Y') }}</td>
                <td class="px-5 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.articles.edit', $article) }}" class="text-blue-900 text-xs font-semibold px-3 py-1.5 border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors">Edit</a>
            ...
            @empty
            <tr><td colspan="4" class="px-5 py-12 text-center text-gray-400">Belum ada artikel.</td></tr>
            @endforelse
        </tbody>
    </table>
